...
class AppController:
    ...
    def load_db_config(self):
        ...
        if os.path.exists(config_path):
            ...
            host = QInputDialog.getText(None, 'Database Configuration', 'Enter database host:', QLineEdit.Normal, 'localhost')[0]
            user = QInputDialog.getText(None, 'Database Configuration', 'Enter database user:', QLineEdit.Normal, 'root')[0]
            password = QInputDialog.getText(None, 'Database Configuration', 'Enter database password:', QLineEdit.Password, '')[0]
            database = QInputDialog.getText(None, 'Database Configuration', 'Enter database name:', QLineEdit.Normal, '')[0]
            db_config = {
                'host': host,
                'user': user,
                'password': password,
    ...
    def show_principal(self, nome_completo, usuario, is_admin):
        self.usuario_atual = usuario
        self.nome_completo = nome_completo
        self.tela_login.close()
        if self.tela_principal:
            self.tela_principal.close()
        self.tela_principal = TelaPrincipal(nome_completo, usuario, is_admin, controller=self, db_config=self.db_config)
        self.tela_principal.show()

    def show_criar_livros(self, usuario):
        if hasattr(self, 'criar_livro'):
            self.criar_livro.close()
        self.criar_livro = CriarLivro(controller=self, db_config=self.db_config)
        self.criar_livro.show()

    def show_listar_livros(self, usuario):
        self.listar_livro = TelaListarLivros(db_config=self.db_config, usuario_atual=self.usuario_atual)
    ...
    def show_marcar_em_lote(self, usuario):
        self.marcar_em_lote = MarcarEmLote(usuario, db_config=self.db_config)
